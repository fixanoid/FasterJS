...
browser.initBadge({ resetTabCount: function(details) {
	...
	updateBadge: function (tabId) {
		...
		if (!tabCounts.hasOwnProperty('tab-' + tabId)) {
			tabCounts['tab-' + tabId] = 0;
		}
...
// TODO: Content security policy prevents data urls from extensions
...
// TODO: if the parent site is HTTPS, we need to skip.
	var parsed = new URL(details.url);
